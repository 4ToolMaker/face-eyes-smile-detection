...
smile_csc = cv2.CascadeClassifier('haarcascade_smile.xml')

# Defining a function that will do the face, eyes and smile detections
...
def detect(gray, frame):
    ...
    for (x, y, w, h) in faces:
        ...
        eyes = eye_csc.detectMultiScale(roi_gray, 1.1, 22)
        for (ex, ey, ew, eh) in eyes:
            cv2.rectangle(roi_color, (ex, ey),
                          (ex + ew, ey + eh), (0, 255, 0), 2)
        smiles = smile_csc.detectMultiScale(roi_gray, 1.8, 22)
